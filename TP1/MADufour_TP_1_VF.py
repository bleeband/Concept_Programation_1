    ...
    # (utilisateur "123" a 2 livres empruntés)
        ...
        while True:
            ...
            if choix_gestL == "1":
                ...
                if not livre_trouve:
                    print(f"{short}\nLe livre {livre_a_emprunter} n'est pas disponible dans ma bibliothèque.")
                    ...
                    for livre in biblio:        #      *** TP PARTIE 3 - VERIFICATION DE DISPONIBILITÉ RETOUR ***
                        if livre["titre"].lower() == livre_a_retourner.lower():
                            ...
                            if livre["statut"] == "emprunté":
                                nouvelle_note = input("Veuillez noter votre livre sur une note de 1 à 5 ★ : ")
                                if nouvelle_note.isdigit() and 1 <= int(nouvelle_note) <= 5:
                                    livre["note"] = int(nouvelle_note)
                                else: 
                                    print("Note invalide, définie à 0")
                                    livre["note"] = 0

                                livre["statut"] = "disponible"
                                livre["client"] = ""
                                note_actuelle = livre["note"]
                                etoiles = "★ " * note_actuelle
                                resume_session.append(f"Retour du livre : {livre['titre'].title()} que vous avez noté {etoiles}")      # *** DEFI 3 - SAUVEGARDE DONNÉES ***
        ...
        while True:
            ...
            if choix_stats == "1":
                ...
                for numero, livre in enumerate(biblio, 1):
                    note_actuelle = livre["note"]
                    etoiles = "★ " * note_actuelle                 
                    print(f"{numero}. {livre['titre'].title()} écrit par {livre['auteur'].title()}   STATUT : {livre['statut']}   NOTE : {etoiles}   CLIENT : {livre['client']}   SORTIE : {livre['nbemprunt']}")
                print()
                
                # AFFICHAGE DES 3 LIVRES LES PLUS EMPRUNTÉ      *** TP PARTIE 4 - TOP 3 avec RANGE() ***
            elif choix_stats == "2":
                print(f"{short}\nTOP 3 des livres les plus emprunté\n")
                biblio.sort(key=lambda x: x['nbemprunt'], reverse=True)
                for i in range(min(3, len(biblio))):
                    livre = biblio[i]
                    note_actuelle = livre["note"]
                    etoiles = "★ " * note_actuelle
                    print(f"{i+1}. {livre['titre'].title()} écrit par {livre['auteur'].title()}   NOTE : {etoiles}   SORTIE : {livre['nbemprunt']}")
                print()

                # AFFICHAGE COUP DE COEUR   *** DEFI 1 - liste 4★ et 5★ ***
            elif choix_stats == "3":    
                print(f"{short}\nLivres COUP DE CŒUR 4 et 5 ★\n")
                livres_top = [livre for livre in biblio if livre["note"] >= 4]
                livres_top.sort(key=lambda x: x["note"], reverse=True)
                ...
                # CLASSEMENT ORDONNÉE
                    ...
                    if choix_tri in cle_dispo:
                        ...
                        for livre in biblio:
                            note_actuelle = livre["note"]
                            etoiles = "★ " * note_actuelle
                            print(f"• {livre['titre'].title()} ecrit par {livre['auteur'].title()}   STATUT :  {livre['statut'].title()}   NOTE : {etoiles}   SORTIE : {livre['nbemprunt']} fois")
                    else:
                        print(choix_inv)
    ...
    #{"titre": "  ", "auteur": "  ", "statut": "  ", "note": "  ", "client": "  ", "nbemprunt": " "}
        ...
        while True:
            ...
            choix_gestB = input(choix_menu)
                ...
                # AJOUTER UN LIVRE A LA BIBLIOTHÈQUE
            if choix_gestB == "1":
                print(f"{short}\nAjouter un livre")
                ...
                resume_session.append(f"Ajout du livre : {nouveau_livre['titre'].title()} ecrit par {nouveau_livre['auteur'].title()}")
                print(f"\n{short}\n\nLe livre suivant : {nouveau_livre['titre'].title()} ecrit par {nouveau_livre['auteur'].title()} a été ajouté à la bibliothèque")

                # ENLEVER UN LIVRE DE LA BIBLIOTHÈQUE
            elif choix_gestB == "2":
                while True:
                    print(f"{short}\nRetirer un livre")
                    ...
                    if index_trouve is not None:
                        ...
                        resume_session.append(f"Retrait du livre : {livre_retire['titre'].title()} ecrit par {livre_retire['auteur'].title()}")
                        print(f"\n{short}\n\nLe livre suivant : {livre_retire['titre'].upper()} ecrit par {livre_retire['auteur'].title()}, a été retiré de votre bibliothèque.")
                        break
                        
                        # SUGGESTTION PROCHE DE LA RECHERCHE
                    else:
                        suggestion = None
                        for livre in biblio:
